...
| 2026-08-20 | 1.5.2 | Přejmenován popisek "Automatické ukládání" na **"Ukládání do prohlížeče"** (`AppToolbar.vue`) — dosavadní text vysvětloval, že jde jen o localStorage, jen v tooltipu (`title`), který na mobilu bez hoveru není vidět; s propojeným souborem na Disku to pak působilo, že appka ukládá při každé změně i tam (ve skutečnosti do souboru zapisuje výhradně tlačítko "💾 Uložit" — `gardenStore._autoSave()` se nikdy nedotkne `fileSourceStore`). |
| 2026-08-20 | 1.6.0 | **Google Disk — pojmenování a přepis.** "Uložit na Disk jako nový soubor" se dřív nezeptal na název (appka si ho vygenerovala sama) — teď se zeptá přes malý dialog (`AppToolbar.vue`, `showSaveAsDialog`), s předvyplněným rozumným výchozím názvem. Nová položka **"Přepsat soubor na Disku…"** (`fileSourceStore.overwriteDriveFile()`) — přes Picker vybereš KTERÝKOLIV existující soubor na svém Disku a appka ho přepíše aktuálním plánem (na rozdíl od "chytrého" Uložit, které přepisuje jen právě navázaný soubor); nezávisí na tom, jestli je plán zatím na nějaký soubor navázaný. **Méně otravné opakované "Propojit se souborem" po refreshi** — jakmile uživatel jednou v rámci otevřeného prohlížeče (tab) potvrdí napojení na konkrétní soubor na Disku, appka si to poznamená do `sessionStorage` (`SESSION_KEY` ve `fileSourceStore.js`) a při dalších refreshích ve STEJNÉ session zkusí tiché načtení (`interactive:false`) bez banneru; nepovede se to (session skutečně skončila, nebo Google odmítl tichý token) → spadne zpět na banner jako dřív. Bezpečnostně beze změny — nikdy nepřepíše rozkreslenou práci potichu bez alespoň jednoho výslovného potvrzení za session. Popisek "Ukládání do prohlížeče" navíc vizuálně odlišen od skutečných tlačítek (bez rámečku/hoveru, kurzívou) a rozšířen na "Ukládá se do prohlížeče (pojistka, ne soubor)", ať je jasné, že nejde o přepínač. |
